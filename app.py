...
# Définit le nombre d'offres d'emploi à analyser par défaut
...
def main_page(client: Client):
    ...
    """
    # Déclaration des éléments pour la barre de recherche et les résultats
    job_input: ui.input = None
    launch_button: ui.button = None
    ...
    # S'assure qu'aucun ancien handler n'est attaché
    session_logger.handlers.clear()
    # Définit le niveau de log à INFO pour un suivi détaillé
    session_logger.setLevel(logging.INFO)

    # Configure le logger racine pour la propagation des messages vers la console du serveur.
    root_logger = logging.getLogger()
    # Ajoute un handler de console seulement si aucun n'est déjà présent
    if not root_logger.handlers:
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(name)s - %(message)s'))
        root_logger.addHandler(console_handler)
    # Définit le niveau minimum de log pour le logger racine
    ...
    # --- En-tête de l'Application (Logo mis à jour) ---
    with ui.header(elevated=True).classes('bg-white text-black px-4 py-2'):
        with ui.row().classes('w-full items-center justify-center'):
            # CODE DU LOGO MIS À JOUR : Ajout du style et de l'événement de clic pour rafraîchir
            ui.image('/assets/SkillScope.svg') \
                .classes('cursor-pointer w-36 h-auto max-w-full transition-transform duration-300 hover:scale-105') \
                .style('max-width: 9rem;') \
                .on('click', refresh_page)
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto p-4 md:p-8 items-center gap-4'):
        ...
        # Déclaration initiale des éléments UI de recherche (cachés au début)
        # Ceci permet de les déclarer avant handle_analysis_click pour la gestion des scopes
        job_input_hidden = ui.input(label='', placeholder='').classes('hidden')
        launch_button_hidden = ui.button('', color='primary').classes('hidden')
        ...
        async def handle_analysis_click():
            ...
            """
            # Utilise la variable globale job_input et launch_button définie ci-dessous
            # Récupère le terme tel qu'entré par l'utilisateur
            original_job_term = job_input.value
            # Normalise le terme pour la logique interne et le cache
            ...
            # Afficher le conteneur des résultats et lui donner les classes
            ...
            # Initialisation du handler de log
        ...
        # Remplace les éléments statiques ou moins dynamiques par l'ancien code plus dynamique
        with ui.column().classes('w-full max-w-lg mx-auto p-0 items-center gap-4'):

            with ui.row().classes('w-full items-stretch'):
                job_input_hidden.delete()
            ...
            launch_button_hidden.delete()
            # Réutilisation de l'ancien code de bouton
        ...
        results_container = ui.column().classes('hidden w-full mt-6')
        ...
        # Cette section est affichée uniquement en mode non-prod pour le débogage
        if not IS_PRODUCTION_MODE:
