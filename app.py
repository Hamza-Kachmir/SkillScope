...
def download_excel_endpoint(client_id: str):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        header_info = pd.DataFrame([['Métier Analysé:', job_title], ['Offres Analysées:', offers_count], []])
...
def download_csv_endpoint(client_id: str):
    ...
    if df is None or df.empty:
        return Response("Aucune donnée à exporter.", media_type='text/plain', status_code=404)
    header_lines = [f"Métier Analysé: {job_title}", f"Offres Analysées: {offers_count}", ""]
...
async def _run_analysis_pipeline(job_input_val: str, logger_instance: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if results is None:
            logger_instance.warning("Le pipeline n'a retourné aucun résultat.")
            return None
        logger_instance.info(f"Fin du pipeline d'analyse pour '{job_input_val}'.")
        return results
    except Exception as e:
        logger_instance.critical(f"ERREUR CRITIQUE DANS LE PIPELINE : {e}", exc_info=True)
        return None

def _store_results_for_client_export(client_id: str, results: Dict[str, Any], job_title_original: str):
    df_to_store = pd.DataFrame([{'classement': i + 1, 'competence': item['skill']} for i, item in enumerate(results.get('skills', []))])
    _export_data_storage[client_id] = {'df': df_to_store, 'job_title': job_title_original, 'actual_offers_count': results.get('actual_offers_count', 0)}
...
def display_results(container: ui.column, results_dict: Dict[str, Any], job_title_original: str):
    ...
    with container:
        ...
        table = ui.table(columns=[{'name': 'classement', 'label': '#', 'field': 'classement', 'align': 'left', 'style': 'width: 10%'}, {'name': 'competence', 'label': 'Compétence', 'field': 'competence', 'align': 'left', 'style': 'width: 70%'},], rows=[], row_key='competence').props('flat bordered').classes('w-full')
...
def main_page(client: Client):
    ...
    ui.add_head_html('''
        ...
        <style>
            ...
            .footer-links a { text-decoration: none !important; color: #2474c5; font-weight: bold; }
            .footer-links a:hover { text-decoration: none !important; }
            .status-banner { width: 100%; max-width: 36rem; color: white; padding: 0.5rem 1rem; border-radius: 0.375rem; text-align: center; font-weight: 500; margin-bottom: 1rem; }
        </style>
    ''')
            ...
            if normalized_job_term in _active_searches:
                ...
                try:
                    ...
                    if results_for_display:
                        ...
                        results_container.clear()
                        with results_container: ui.label(f"Aucun résultat trouvé pour '{original_job_term}'. Veuillez réessayer.").classes('text-negative')
                except Exception as e:
                    results_container.clear()
                    with results_container: ui.label(f"Une erreur est survenue lors de l'attente de l'analyse : {e}").classes('text-negative')
                finally:
        ...
        if not IS_PRODUCTION_MODE:
            with ui.expansion("Voir les logs & Outils", icon='o_code').classes('w-full mt-12 bg-gray-50 rounded-lg'):
                with ui.column().classes('w-full p-2'):
                    ...
                    # Le handler est maintenant ajouté conditionnellement au logger de la session
                    # pour éviter d'avoir un handler sans cible au démarrage.
